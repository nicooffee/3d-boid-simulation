...
class Boid:
    ...
    async def flock(self,flock):
        async def _async_flock(b,b2):
            s_a = np.zeros(3) #vector alineamiento
            s_c = np.zeros(3) #vector cohesion
            s_r = np.zeros(3) #vector separacion
            d = b.distancia(b2)
            if b2.id != b.id and b.en_rango(b2):
                s_a = s_a + b2.velocidad
                s_c = s_c + b2.posicion
                dif = b.posicion - b2.posicion
                s_r = s_r + np.divide(dif,d if d>0 else 0.00001)
            return (s_a,s_c,s_r)
        ...
        result = await asyncio.ensure_future(asyncio.gather(*tasks))
        boid_added = len(result)
        print(boid_added)
        boid_in_range = boid_added
        print(result)
        (suma_a,suma_c,suma_r) = ft.reduce(lambda t1,t2: (t1[0]+t2[0],t1[1]+t2[1],t1[2]+t2[2]),result,(np.zeros(3),np.zeros(3),np.zeros(3)))
        #print(suma_a,suma_c,suma_r)
        '''
        for boid in flock:
        ...
        '''
        cmpa = suma_a != np.zeros(3)
        cmpc = suma_c != np.zeros(3)
        cmpr = suma_r != np.zeros(3)
        if cmpa.all() and cmpc.all() and cmpr.all():
        ...
        #print(suma_a,suma_c,suma_r,'\n\n')
        return suma_a+suma_r*1.1+suma_c
